...
class DataSetLoader(object):
    ...
    def __init__(self, name, device, mix_cpu_gpu=False,
        ...
        if self._name in ['ml-100k', 'ml-1m', 'ml-10m']:
        ...
        elif self._name in ['yahoo_music']:
            self._dir = os.path.join(_paths[self._name])
            rating_map = None
            testing = True
        ...
        elif self._name in ['yelp']:
            print("yyyyyy")
            path_dataset = 'raw_data/' + 'yelp' + '/yelp_train.json'
            train_u_indices = []
            train_v_indices = []
            train_labels = []
            with open(path_dataset, 'r', encoding='utf-8') as f:
            ...
            path_dataset = 'raw_data/' + 'yelp' + '/yelp_valid.json'
            valid_u_indices = []
            valid_v_indices = []
            valid_labels = []
            with open(path_dataset, 'r', encoding='utf-8') as f:
            ...
            path_dataset = 'raw_data/' + 'yelp' + '/yelp_test.json'
            test_u_indices = []
            test_v_indices = []
            test_labels = []
            with open(path_dataset, 'r', encoding='utf-8') as f:
            ...
            self.min_l = np.min(self.possible_rating_values)
            print("self.possible_rating_values")
            print(self.possible_rating_values, self.possible_rating_values.shape)
        else:
            raise NotImplementedError
        ...
        # print(test_setu - train_setu, len(test_setu - train_setu))
        # print(test_setm - train_setm, len(test_setm - train_setm))
        # print("trainpair_u")
        # for i, t in enumerate(train_rating_pairs[0]):
        #     if t == 0:
        #         print(train_rating_pairs[0][i], train_rating_pairs[1][i], train_rating_values[i])
        # print("testpair_u")
        # for i, t in enumerate(test_rating_pairs[0]):
        #     if t == 0:
        #         print(test_rating_pairs[0][i], test_rating_pairs[1][i], test_rating_values[i])
        # print("trainpair_v")
        # for i, t in enumerate(train_rating_pairs[1]):
        #     if t == 1076:
        #         print(train_rating_pairs[1][i], train_rating_pairs[0][i], train_rating_values[i])
        # print("testpair_v")
        # for i, t in enumerate(test_rating_pairs[1]):
        #     if t == 1076:
        #         print(test_rating_pairs[1][i], test_rating_pairs[0][i], test_rating_values[i])
        ...
        # user_movie = np.zeros((self._num_user, self._num_movie), dtype=np.int32)
        # user_movie[all_train_rating_pairs] = all_train_rating_values
        # movie_user = user_movie.T
        # nums_u = th.zeros((self._num_user, len_rate))
        # nums_m = th.zeros((self._num_movie, len_rate))
        # self.u_similar = th.zeros((self._num_user, 3), dtype=th.long)
        # self.u_dissimilar = th.zeros((self._num_user, 3), dtype=th.long)
        # self.i_similar = th.zeros((self._num_movie, 3), dtype=th.long)
        # self.i_dissimilar = th.zeros((self._num_movie, 3), dtype=th.long)
        # #print(min(all_train_rating_values))
        # # print(len(self.possible_rating_values))
        # for i in range(self._num_user):
        #     for j in range(self._num_movie):
        #         if user_movie[i][j] == 0:
        #             continue
        #         nums_u[i][user_movie[i][j] - 1] += 1
        # numall_u = nums_u.sum(dim=1)
        
        # for i in range(self._num_movie):
        #     for j in range(self._num_user):
        #         if movie_user[i][j] == 0:
        #             continue
        #         nums_m[i][movie_user[i][j] - 1] += 1
        # numall_m = nums_m.sum(dim=1)
        

        # a = nums_u
        # b = a
        # print("COS")
        # print(nums_u.shape, nums_m.shape)
        # x = a.unsqueeze(1)
        # y = b.unsqueeze(0)
        # cosine_similarity = F.cosine_similarity(x, y, dim=-1)
        # # print(cosine_similarity, cosine_similarity.shape)
        # print(nums_u[24], nums_u[1], nums_u[10])
        # print(nums_u[240], nums_u[3], nums_u[203])
        # # 对每一个用户
        # for i in range(cosine_similarity.shape[0]):

        #     # 获得最相似的五个用户
        #     _, top_3_similar_users_idxs = th.topk(cosine_similarity[i], k=3, sorted=True)
        #     self.u_similar[i] = top_3_similar_users_idxs

        #     # 获得最不相似的五个用户 
        #     _, top_3_dissimilar_users_idxs = th.topk(cosine_similarity[i], k=3, largest=False, sorted=True)
        #     self.u_dissimilar[i] = top_3_dissimilar_users_idxs

        # #print(self.u_similar, self.u_similar.shape)
        ...
        # b1 = a1
        # print("COS_m")
        # #print(nums_m.shape, nums_m.shape)
        # x1 = a1.unsqueeze(1)
        # y1 = b1.unsqueeze(0)
        # cosine_similarity1 = F.cosine_similarity(x1, y1, dim=-1)
        # print(nums_u[195], nums_u[4], nums_u[15])
        # # print(nums_u[240], nums_u[3], nums_u[203])
        # # 对每一个用户
        # for i in range(cosine_similarity1.shape[0]):

        #     # 获得最相似的五个用户
        #     _, top_3_similar_items_idxs = th.topk(cosine_similarity1[i], k=3, sorted=True)
        #     self.i_similar[i] = top_3_similar_items_idxs

        #     # 获得最不相似的五个用户 
        #     _, top_3_dissimilar_users_idxs = th.topk(cosine_similarity[i], k=3, largest=False, sorted=True)
        #     self.i_dissimilar[i] = top_3_dissimilar_users_idxs

        #print(self.i_similar, self.i_similar.shape)
    ...
    def _generate_enc_graph(self, rating_pairs, rating_values, add_support=False, device=None,dataset = None):
        ...
        # len_rate = 100
        # user_movie = np.zeros((self._num_user, self._num_movie), dtype=np.int32)
        # user_movie[rating_pairs] = rating_values
        # movie_user = user_movie.T
        # nums_u = th.zeros((self._num_user, len_rate))
        # nums_m = th.zeros((self._num_movie, len_rate))
        ...
        # numall_m = nums_m.sum(dim=1)
        #
        # # print(nums_u, nums_u.shape)
        # # print(numall_u, numall_u.shape)
        # # print(nums_m, nums_m.shape)
        # # print(numall_m, numall_m.shape)
        # rates_u = th.zeros((self._num_user, len_rate), dtype=th.float32).to(device)
        # rates_m = th.zeros((self._num_movie, len_rate), dtype=th.float32).to(device)
        #
        # for i in range(self._num_user):
        ...
        #             rates_u[i][j] = 0
        #         else:
        #             rates_u[i][j] = nums_u[i][j] / numall_u[i]
        # for i in range(self._num_movie):
        #     for j in range(len_rate):
        #         if numall_m[i] == 0:
        #             rates_m[i][j] = 0
        #         else:
        #             rates_m[i][j] = nums_m[i][j] / numall_m[i]
        # print("rate")
        # print(rates_u, rates_u.shape)
        # print(rates_m, rates_m.shape)
...
def generate_G_from_H(H, variable_weight=False):
   ...
   #print('guiyihua')
    ...
    # print("1e-12")
    # print(np.isnan(invDE).any)
    # print(np.isnan(DV2).any)
